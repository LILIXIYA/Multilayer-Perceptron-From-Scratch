...
class NeuronlGraph:
    ...
    def __init__(self, X, Y, INI, Num_inputLayer=2, Num_hiddenLayer=2, \
                Size_hiddenLayer=[10,10], Num_outputLayer=1, lr = 0.001):
        # Stop thresold
        self.thr = 1e-1
        # Learning rate
        self.lr = lr
    ...
    def Initialization(self, INI, Num_inputLayer, Size_hiddenLayer, Num_outputLayer):
        if INI == 'Normal':
            ...
            self.w_2 = np.random.random_sample((Size_hiddenLayer[0], Size_hiddenLayer[1]))
            self.w_3 = np.random.random_sample((Size_hiddenLayer[1], Num_outputLayer))
            # Initialize the bias
            self.b_1 = np.random.random_sample(Size_hiddenLayer[0])
            self.b_2 = np.random.random_sample(Size_hiddenLayer[1])
        ...
        if INI == 'Gaussian':
            ...
            '''
            #Settings for HW31
            self.w_1 = np.zeros((Num_inputLayer, Size_hiddenLayer[0]))
            self.w_2 = np.zeros((Size_hiddenLayer[0], Size_hiddenLayer[1]))
            self.w_3 = np.zeros((Size_hiddenLayer[1], Num_outputLayer))
            # Initialize the bias
            self.b_1 = np.zeros(Size_hiddenLayer[0])
            self.b_2 = np.zeros(Size_hiddenLayer[1])
            self.b_3 = np.zeros(Num_outputLayer)
            '''
    ...
    def feedforward(self):
        ...
        self.output = sigmoid(np.dot(self.h2, self.w_3) + self.b_3)
