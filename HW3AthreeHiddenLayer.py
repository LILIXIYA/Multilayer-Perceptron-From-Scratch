...
class NeuronlGraph:
    ...
    def __init__(self, X, Y, INI, Num_inputLayer=2, Num_hiddenLayer=3, \
                Size_hiddenLayer=[10,10,10], Num_outputLayer=1, lr = 0.01):
        # Stop thresold
        self.thr = 1e-1
        # Learning rate
        self.lr = lr
    ...
    def Initialization(self, INI, Num_inputLayer, Size_hiddenLayer, Num_outputLayer):
        ...
        if INI == 'Gaussian':
            ...
            self.w_2 = np.random.normal(loc=0, scale=sigma, size=(Size_hiddenLayer[0], Size_hiddenLayer[1]))
            self.w_3 = np.random.normal(loc=0, scale=sigma, size=(Size_hiddenLayer[1], Size_hiddenLayer[2]))
            self.w_4 = np.random.normal(loc=0, scale=sigma, size=(Size_hiddenLayer[2], Num_outputLayer))
            # Initialize the bias
            self.b_1 = np.random.normal(loc=0, scale=sigma, size=Size_hiddenLayer[0])
            self.b_2 = np.random.normal(loc=0, scale=sigma, size=Size_hiddenLayer[1])
            self.b_3 = np.random.normal(loc=0, scale=sigma, size=Size_hiddenLayer[2])
            self.b_4 = np.random.normal(loc=0, scale=sigma, size=Num_outputLayer)
            
    def feedforward(self):
        self.h1 = relu(np.dot(self.input, self.w_1) + self.b_1)
        self.h2 = relu(np.dot(self.h1, self.w_2) + self.b_2)
        self.h3 = relu(np.dot(self.h2, self.w_3) + self.b_3)
        
        self.output = sigmoid(np.dot(self.h3, self.w_4) + self.b_4)
       
    def backprop(self,loss_type):
        # derivative of weights
        if loss_type == 'MSE':
            d_temp0 = 2*(self.y - self.output) * sigmoidDerivative(self.output)
            
        d_w4 = np.dot(self.h3.T, d_temp0)
        
        d_temp1 = np.dot(d_temp0, self.w_4.T) * reluDerivative(self.h3)
        d_w3 = np.dot(self.h2.T, d_temp1)
        
        d_temp2 = np.dot(d_temp1, self.w_3.T) * reluDerivative(self.h2)
        d_w2 = np.dot(self.h1.T, d_temp2)
        
        d_temp3 = np.dot(d_temp2, self.w_2.T) * reluDerivative(self.h1)
        d_w1 = np.dot(self.input.T, d_temp3)
   
        # update weights
        self.w_1 += d_w1 * self.lr
        self.w_2 += d_w2 * self.lr
        self.w_3 += d_w3 * self.lr
        self.w_4 += d_w4 * self.lr
        
        # derivate of bias
        d_b4 = np.sum(d_temp0, axis=0)
        d_b3 = np.sum(d_temp1, axis=0)
        d_b2 = np.sum(d_temp2, axis=0)  
        d_b1 = np.sum(d_temp3, axis=0)  
        # update bias
        self.b_4 += d_b4 * self.lr
        self.b_1 += d_b1 * self.lr
        self.b_2 += d_b2 * self.lr
